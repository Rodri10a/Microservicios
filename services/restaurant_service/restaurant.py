...
# sys y os: para agregar la carpeta padre al path y poder importar common/
import sys, os, datetime # fecha de expiracion del token JWT
...
app = Flask(__name__) 
# Credenciales demo para autenticacion
DEMO_USER = {"username": "Rorro", "password": "rorro123", "role": "admin"}


# --- Endpoints (3) ---

# Autentica al usuario y devuelve un token JWT con expiracion de 8 horas.
@app.route("/auth/token", methods=["POST"])
def login():
    datos = request.get_json() 
    # si los datos son incorrectos retorna incorrectas 
    if not datos or datos.get("username") != DEMO_USER["username"] or datos.get("password") != DEMO_USER["password"]:
        return jsonify({"error": "Credenciales incorrectas"}), 401 # no autenticado
...
# GET: devuelve items del menu. 
# POST: agrega un nuevo item al menu.
...
def menu():
    ...
    with get_db() as conn:
        cur = conn.execute("INSERT INTO menu_items (restaurant_name, name, price) VALUES (?, ?, ?)",
                        (datos["restaurant_name"], datos["name"], datos["price"])) # se inserta los datos en vez de ? 
        conn.commit()
        # lastrowid me da el numero de la ID 
    return jsonify({"id": cur.lastrowid, "restaurant_name": datos["restaurant_name"],
                    "name": datos["name"], "price": datos["price"]}), 201


# Endpoint interno: devuelve un item por ID.
# Usado por order_service para validar items al crear un pedido.
# SOLO PUEDEN USAR LOS MICROSERVICIOS 
...
def item_interno(item_id):
    ...
    return jsonify(dict(item)) # convierte a un diccionario 
